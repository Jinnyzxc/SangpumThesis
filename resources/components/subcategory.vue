        ...
        <div class="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
            <div class="bg-white shadow w-full rounded-lg divide-y divide-gray-200">
                ...
                <form method="post" @submit.prevent="submit">
                    <div class="px-5 py-3">
                        <h1 class="font-bold text-center text-2xl mb-5"> Add New Sub Category </h1>
                        <span class=" flex justify-center text-center"></span>
                    </div>
                    <div class="px-5 py-3">
                        <input type="text" name="subcategory_name" placeholder="Sub_Category Name" id="subcategory_name" v-model="formInput.sub_categoryname"
                            class="border rounded-full px-3 py-2 mt-1 mb-5 text-sm w-full" required />
                        <input type="text" name="subcategory_description" placeholder="SubCategory_Description" id="subcategory-description" v-model="formInput.sub_categorydescription"
                            class="border rounded-full px-3 py-2 mt-1 mb-5 text-sm w-full" required />  
                        <input type="text" name="parent_category" placeholder="Parent_Category" id="parent_category" v-model="formInput.parent_category"
                            class="border rounded-full px-3 py-2 mt-1 mb-5 text-sm w-full" required />                         
...
export default {
    ...
    methods: {
        async submit() {
            try {
                const result = await axios.post('/api/sub_category/add', this.formInput);
