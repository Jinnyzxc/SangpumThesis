        ...
        <div class="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
            <div class="bg-white shadow w-full rounded-lg divide-y divide-gray-200">
                ...
                <form method="post" @submit.prevent="submit">
                    ...
                    <div class="px-5 py-3">
                        <input type="text" name="brand-name" placeholder="Brand Name" id="brand-name" v-model="formInput.brand_name"
                            class="border rounded-full px-3 py-2 mt-1 mb-5 text-sm w-full" required />
                        <input type="text" name="brand-description" placeholder="Brand Description" id="category-description" v-model="formInput.brand_description"
                            class="border rounded-full px-3 py-2 mt-1 mb-5 text-sm w-full" required />                       
...
export default {
    ...
    methods: {
        async submit() {
            try {
                const result = await axios.post('/api/brand/add', this.formInput);
